...
export class CardListComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  onDrop(event: CdkDragDrop<ICards[]>) {
    ...
    if (event.container.id === 'todo_id' && event.item.data.status === CARD_STATUS.DONE) {
      event.item.data.status = CARD_STATUS.REJECTED;
    } else if (event.container.id === 'done_id' && (event.item.data.status === CARD_STATUS.PENDING ||
      event.item.data.status === CARD_STATUS.REJECTED)) {
  ...
  getCards() {
    this.cardService.searchCards()
      ...
      .subscribe( (cards: ICards[]) => {
        this.cards = cards;
        this.pendingCards = this.cards.filter(card => (card.status === CARD_STATUS.PENDING
          || card.status === CARD_STATUS.REJECTED));
